...
10. **Fontes heterogêneas:** a fila e o ingresso (`/api/ingest`) recebem não só eventos de câmeras, mas de **dispositivos/sensores** (ex.: alagamento, vazamento, temperatura) — qualquer origem que produza um evento tipado. O modelo de evento carrega `device_type` (`camera` | `sensor` | …) e `event_type`/contexto genéricos; a avaliação (regras) é por **tipo de evento**, independente da origem. A Visão geral é centrada em câmeras; eventos de dispositivos entram na lista de Eventos e nas regras de providência.
